...
void avlTreeInit2(void)
{
  OS_ERR      err;
  OS_MEM_QTY node_size = sizeof(CPU_INT32U);
  OSMemCreate((OS_MEM*)&MemoryCB_avl2, (CPU_CHAR*)"avl_tree_node", &MemoryPartition_avl2[0][0], (OS_MEM_QTY)(10), (OS_MEM_SIZE)(3*node_size), &err);
  avl_root2 = NULL;
  avl_min2 = NULL;
}
...
AVL_NODE2* insert2(AVL_NODE2* node,  OS_TCB* tcb_pointer, CPU_INT08U preemption_threshold)
{
    ...
    if (preemption_threshold <= node->preemption_threshold)
        node->left  = insert2(node->left, tcb_pointer, preemption_threshold);
    else
        node->right = insert2(node->right, tcb_pointer, preemption_threshold);
    ...
    avl_min2 = minValueNode2(avl_root2);
    /* return the (unchanged) node pointer */
    return node;
}


AVL_NODE2 * minValueNode2(AVL_NODE2* node)
{
...
AVL_NODE2* deleteNode2(AVL_NODE2* root,  CPU_INT08U preemption_threshold)
{
    ...
    if ( preemption_threshold <= root->preemption_threshold )
        root->left = deleteNode2(root->left, preemption_threshold);
    
    else if( preemption_threshold > root->preemption_threshold )
        root->right = deleteNode2(root->right, preemption_threshold);
    ...
    else
    {
        ...
        else
        {
            ...
            AVL_NODE2* temp = minValueNode2(root->right);
            
            // Copy the inorder successor's data to this node
            root->preemption_threshold = temp->preemption_threshold;
            
            // Delete the inorder successor
            root->right = deleteNode2(root->right, temp->preemption_threshold);
        }
    }
